...
object WorldGenerator {
    ...
    fun generateNewPlot(plotName: String, shape: GenerationShapes, endSessionRunnable: (PlotState) -> Unit = {}) {
        if (plotName.isBlank()) {
            throw Exception("Blank name cannot be accepted")
        }
        val createdPlot = PlotState.registerNewPlot(plotName, 1.0f)
        generatePlotTerrain(createdPlot, shape, endSessionRunnable)
    }

    fun generateNewPlot(plotName: String, coordinates: Pair<Int, Int>, shape: GenerationShapes, endSessionRunnable: (PlotState) -> Unit = {}) {
        if (plotName.isBlank()) {
            throw Exception("Blank name cannot be accepted")
        }
    ...
    fun generatePlotTerrain(plot: PlotState, shape: GenerationShapes, endSessionRunnable: (PlotState) -> Unit = {}) {

        val seed = Random.nextInt()
        val generationArea = CuboidRegion.fromCenter(plot.getCuboidRegion().center.toBlockPoint(), 30)
        ...
        val generationTasks: List<(EditSession) -> Unit> =
            listOf(
                ...
                { _ -> endSessionRunnable.invoke(plot) }
            )
